...
写作硬性要求（直接决定报告是否物有所值）：
1. 逐国方案必须各不相同：结合每个国家的真实移民逻辑(打分项/门槛/职业清单/资金要求)与该用户的具体差距来写，严禁多国套用同一套话术。
2. 各部分不要重复：整体判断、逐国方案、风险三处给不同角度的信息增量。
3. 要具体、可执行：少说"建议提升语言"这类空话，多给"考到什么分、做什么认证、补什么材料、为什么有用"。
4. 必须审慎：体现政策会变、以官方与持牌顾问为准；不得承诺获批、不得给获批百分比、不得出现"保证/稳过/包过/100%"。
...
请严格按以下 JSON 输出（countryPlans 针对这些国家，名称精确匹配：${JSON.stringify(countries)}）：
{
 ...
 "countryPlans":[{"country":"<精确国名>","recommendedPath":"推荐路径名","whyPath":"为什么走这条(结合该国逻辑与用户条件)","keyThresholds":["关键门槛1","门槛2"],"timeline":["按阶段/月的时间线1","2"],"costRange":"费用预算区间(人民币，注明大致含义)","improvementToQualify":["要达标需要做的具体动作1","2"],"risks":[{"severity":"高|中|低","risk":"string","howToReduce":"string"}],"docChecklist":["材料1","材料2"]}],
...
factorAdvice 给 2–4 条；countryPlans 覆盖上面列出的国家；每个 keyThresholds/timeline/improvementToQualify 2–4 条。` },
  ];
  let out;
